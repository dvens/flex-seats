<section class="calendar js--calendar">
	...
	<ul class="calendar__dates">
		...
		<?php forEach($dates as $key => $date) {  ?>
			...
			<li <?php echo ($date['disabled'] ? 'disabled' : ''); ?>  class="calendar__date calendar__date--<?php echo $date['status']; ?> <?php echo (matchDay($date['date'], $fullDate) ? 'is--active' : ''); ?>">
				...
				<a class="<?php echo (isCurrentDay($date['date']) ? 'is--today' : '');?>" href="<?php echo '?page=home&month='. $month . '&year='. $year . '&day='. $date['day-full'] ?>">
					...
					<?php } else { ?>
						...
						<div class="calendar__desk-holder">
							...
							<?php if($amountDesks - $date['employees'] >= 1 && !matchDay($date['date'], $fullDate) ) { ?>
							...
							<?php } elseif($amountDesks - $date['employees'] < 1 && !matchDay($date['date'], $fullDate)) { ?>
					...
					<section class="calendar__content">
						...
						<?php if( $date['status'] === 'office') { ?>
							...
							<form action="?page=<?php echo $page; ?>" method="post">
								...
								<button name="action" value="yes" type="submit" class="button--highlighted">i'm working from home</button>	
							</form>
						...
						<?php } else { ?>
							...
							<form action="?page=<?php echo $page; ?>" method="post">
								<input name="date" type="hidden" value="<?php echo $date['date']; ?>">
								<button name="action" value="no" type="submit" class="button--highlighted">i'm working at the office</button>	
							</form>	
